...
public class Bech32 {
    ...
    public static Bech32Data decode(final String str) throws Exception {
        ...
        for (int i = 0; i < str.length(); ++i) {
            char c = str.charAt(i);
            if (c < 33 || c > 126) throw new Exception("Invalid character " + c + " at position " + i);
            if (c >= 'a' && c <= 'z') {
                if (upper)
        ...
        if (dataPartLength < 6) throw new Exception("Data part too short: " + dataPartLength);
        byte[] values = new byte[dataPartLength];
        for (int i = 0; i < dataPartLength; ++i) {
            char c = str.charAt(i + pos + 1);
            if (CHARSET_REV[c] == -1) throw new Exception("Invalid character " + c + " at position " + (i + pos + 1));
